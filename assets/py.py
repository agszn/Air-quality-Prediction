...
# Define the assets folder within the current directory (you can modify the folder name if needed)
assets_folder = os.path.join(current_directory, 'images')
    ...
    if os.path.exists(assets_folder) and os.path.isdir(assets_folder):
        ...
        for filename in os.listdir(assets_folder):
            ...
            if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp', '.svg')):
                ...
                readme.write(f"![{description}]({os.path.join('assets', filename)})\n\n")
